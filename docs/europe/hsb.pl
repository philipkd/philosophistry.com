...
print tuple2hex($red,$green,$blue);
print "\n\n";

open FILE, ">colors.php";
select (FILE);
$min = 11;
$max = 55;
...
#	my ($red, $green, $blue) = HSBtoRGB(60,($i+1)*10,100);
	my ($red, $green, $blue) = HSBtoRGB(60,(($i-$min)/($max-$min))*100,50+(($i-$min)/($max-$min))*50);
